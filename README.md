...
## How to check it
The evidence a reader uses to check the auditor lives in the folders on the never-load list: `fixtures/` (anonymised notes, among them four real notes of a client of the office carrying a real wrong-code FAIL, plus one-field mutations and one violation kept while the text around it is reworded three ways), `expected/` (what each fixture must return, committed before the run), [`rounds/`](rounds/) (the office's recorded rounds, a walk by someone from outside the trade who ran it alone, the author's fixture runs, the controls and the refusal, each with what it does not prove) and [`tools/`](tools/). `tools/README.md` lists each claim this folder makes, the command that breaks it if it is false, and how to break it on purpose.
