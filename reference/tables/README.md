...
**Known mismatches, counted on 07/09/2026 by script:** the second table uses 675 distinct NBS codes, of which 582 exist in the official NBS list (ANEXO_B); its `indop` column has 24 distinct values, 11 of them written exactly as in ANEXO_C (the others look like the same codes with leading zeros dropped). Nothing was corrected: the tables are shipped as received, hashed in `working/SHA256SUMS.txt`.
