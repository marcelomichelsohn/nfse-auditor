...
**Why the author's fixture runs of 09/09 are not the v1 baseline (added 11/09/2026, before the first run, after the author's assistant checked the two folders).** `rounds/fixture-runs-v1/report-all-runs.md` declares its conditions: the Claude desktop app, and a project holding the files `CLAUDE.md` lists *except* `README.md` and `examples.md`, which did not yet exist in usable form. The runs here are on claude.ai in Chrome, with all 48 files of each version, README and examples included. Two differences, surface and file set; so the 4 min 44 s to 5 min 13 s of 09/09 do not enter as the v1 side — v1 is re-run here under the same conditions as v2, which is what this control isolates. The two folders were checked from outside this session: v1 rebuilt from `ac96786` and v2 from HEAD, each `diff -rq` identical to the folders uploaded; v2 identical to the unpacked `download/nfse-auditor-carregar.zip`; v1 and v2 differ in exactly five files (`CLAUDE.md`, `README.md`, `examples.md`, `identity.md`, `rules.md`), the other 43 byte-identical.

**A run before the authorisation, declared.** One v1 run was started at 11:06:54 on 11/09, before the author had said where the runs would happen; it is recorded in `transcript.md` as run 0 and does not count. The eight runs of the design start after it.
