...
> Snapshot of the study (10/09/2026). The labels and the column header `check` in this file are the ones the decision was made on; after it, `rules.md` § 3 reworded three labels and made the Portuguese header `verificação`. The vocabulary that governs is `rules.md`; this file is evidence of the choice, kept as it was. **The numbers are a snapshot too, and are not re-derived (said 11/09/2026).** Every count here — the rows measured, the widths, the line numbers cited as `l.NNN`, and the four `Re-run` commands at the end — describes the repository as it stood on 10/09. Three of those commands print something else today, because `expected/` gained files, the English tables left `examples.md`, and the files they read have grown. Re-running them would replace the evidence of a past decision with a measurement of a different repository, so they are left exactly as they were, and this line is the warning instead.
