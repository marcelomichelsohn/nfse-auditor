...
- **The client profile line** (one line, pasted by the operator, from the office's control sheet), in this form:
  ...
  Without it, the checks that need it return CANNOT DETERMINE and say so; nothing is guessed from the note. The note's own flags (`opSimpNac`, `regApTribSN`) and its free text (`xOutInf`) say what the note claims about itself; they do not replace the profile line. **Without a profile line, `1-código`, `4` and `5-alíquota` are CANNOT DETERMINE** (found on 09/09/2026, example 4 of `examples.md`).
