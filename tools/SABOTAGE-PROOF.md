# SABOTAGE-PROOF — 2026-09-12T00:01:44-03:00 — HEAD 7f268cf
...
- CAUGHT  no real identifier (C6) — `FAIL C6  rules.md:50: check-digit-valid CNPJ in text: [the planted CNPJ]`
- CAUGHT  predictions before reports, from git (prove_order) — `(i) round-9-sabotage: expected (d0fa3a1) before transcript (944f3de)`
- CAUGHT  the checker is not decoration (--selftest) — `SELFTEST FAIL`
- CAUGHT  the README's own quotes are the standard's text (C1) — `FAIL C1  README.md:11: quote does not resolve in reference/pt/: “17.19 - a sentence that is not in the law.…”`
- CAUGHT  a clean fixture's prediction never claims a failure (C7) — `FAIL C7  expected/nfse-01.md:10: a clean fixture's prediction claims a failure`
